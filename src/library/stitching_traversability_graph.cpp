 ...
 *          2) stiching this local image in the full image
 *                  using kernel approach to give less weight to border points
...
void TraversabilityGraphStitcher::pointCloudCallback(const sensor_msgs::PointCloud2::Ptr& pcd_msg)
{
    ...
    if(cloud->points.size() > 600)
    {
        ...
        gammaCorrection(elevation_tmp, elevation_tmp, gamma_); //2.0 too much, 1.0 not enougth
        time2 = ros::Time::now();
